...
public final class NhaXuatBan extends JPanel implements ActionListener, ItemListener {

    PanelBorderRadius main, functionBar;
    JPanel pnlBorder1, pnlBorder2, pnlBorder3, pnlBorder4, contentCenter;
    JTable tableNhaXuatBan;
    ...
    public ArrayList<NhaXuatBanDTO> listnxb = nxbBUS.getAll();
    ...
    private void initComponent() {
        ...
        tableNhaXuatBan.setModel(tblModel);
        tableNhaXuatBan.setFocusable(false);
        scrollTableSanPham.setViewportView(tableNhaXuatBan);
        DefaultTableCellRenderer centerRenderer = new DefaultTableCellRenderer();
        centerRenderer.setHorizontalAlignment(JLabel.CENTER);
        TableColumnModel columnModel = tableNhaXuatBan.getColumnModel();
        ...
        search = new IntegratedSearch(new String[]{"Tất cả", "Mã nhà xuất bản", "Tên nhà xuất bản", "Địa chỉ", "Email", "Số điện thoại"});
        ...
        search.txtSearchForm.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                listnxb = nxbBUS.search(txt, type);
                loadDataTable(listnxb);
            }
        });
...
//        main.setBorder(new EmptyBorder(20, 20, 20, 20));
    ...
    public NhaXuatBan(Main m) {
        this.m = m;
        initComponent();
        tableNhaXuatBan.setDefaultEditor(Object.class, null);
        loadDataTable(listnxb);
    }

    public void loadDataTable(ArrayList<NhaXuatBanDTO> result) {
        tblModel.setRowCount(0);
        for (NhaXuatBanDTO nxb : result) {
            tblModel.addRow(new Object[]{
                nxb.getManxb(), nxb.getTennxb(), nxb.getDiachi(), nxb.getEmail(), nxb.getSdt()
            });
        }
    }
    ...
    public int getRowSelected() {
        int index = tableNhaXuatBan.getSelectedRow();
        if (index == -1) {
        ...
        if (e.getSource() == mainFunction.btn.get("create")) {
            ...
            if (index != -1) {
                int input = JOptionPane.showConfirmDialog(null,
                        "Bạn có chắc chắn muốn xóa nhà xuất bản!", "Xóa nhà xuất bản",
                        JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE);
                if (input == 0) {
                    nxbBUS.delete(listnxb.get(index), index);
                    loadDataTable(listnxb);
                }
            }
            ...
            try {
                JTableExporter.exportJTableToExcel(tableNhaXuatBan);
            } catch (IOException ex) {
